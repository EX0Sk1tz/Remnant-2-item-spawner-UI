...
public sealed class MainViewModel : INotifyPropertyChanged
{
    ...
    private bool _aimMagicBulletsEnabled;
    private double _aimFov = 5.0;
    ...
    public MainViewModel()
    {
        ...
        // multi-MB UE4SS.log read), so don't block window construction on it.
        _ = RunStartupDiagnosticsAsync();

        // Cheats (and the movement speed multiplier below) intentionally do NOT load whatever was
        ...
        _aimMagicBulletsEnabled = cheatSettings.AimMagicBulletsEnabled;
        _aimFov = cheatSettings.AimFov <= 0 ? 5.0 : cheatSettings.AimFov;
        ...
        // Same "always off/default at launch" rule as the cheats above -- unlike the rest of
        // HotkeySettings (bindings, language, wiki, ...), which are app preferences and stay as
        // the user left them, MovementSpeedMultiplier is a gameplay cheat that happens to be
        // stored in this file. Force it back to 1.0 and persist that correction immediately so
        // movement_speed.lua also settles at "inactive" instead of resuming the old multiplier.
        _movementSpeedMultiplier = 1.0;
        settings.MovementSpeedMultiplier = _movementSpeedMultiplier;
    ...
    private async Task<bool> RefreshSummonableTraitsStateAsync()
    {
        // The underlying check walks the Mods folder and reads every main.lua it finds, so it's
        // real disk I/O -- run it off the UI thread and only touch bound items once we have the answer.
        var installed = await Task.Run(() => _summonableTraitsService.IsInstalled());

        IsSummonableTraitsInstalled = installed;

        foreach (var item in _allItems)
            item.IsSummonableTraitsInstalled = installed;
    ...
    private async Task RunStartupDiagnosticsAsync()
    {
        ...
        {
            AppLogService.Error("Startup diagnostics refresh failed", ex);
        }
    }

    public async Task RefreshDiagnosticsAsync()
    {
        var traitsInstalled = await RefreshSummonableTraitsStateAsync();

        // DiagnosticsService.Run() does a full pass of File/Directory checks, JSON parsing, and
        // (when the game isn't running) reads the whole UE4SS.log -- keep that off the UI thread too.
        _diagnosticReport = await Task.Run(() => _diagnosticsService.Run(traitsInstalled));
    ...
    private CheatSettings BuildCheatSettings() => new()
    {
        InfiniteHealth = InfiniteHealth,
        InfiniteStamina = InfiniteStamina,
        InfiniteAmmo = InfiniteAmmo,
        NoFallDamage = NoFallDamage,
        AimMagicBulletsEnabled = AimMagicBulletsEnabled,
        AimFov = AimFov,
    ...
    private void SaveCheatSettings()
    {
        try
        {
            ...
            StatusText = $"Cheat settings saved. Health={InfiniteHealth}, Stamina={InfiniteStamina}, Ammo={InfiniteAmmo}, NoFallDamage={NoFallDamage}";
        }
        catch (Exception ex)
        {
    ...
    public bool AimMagicBulletsEnabled
    {
        get => _aimMagicBulletsEnabled;
        set
        {
            if (_aimMagicBulletsEnabled == value)
                return;

            _aimMagicBulletsEnabled = value;
            OnPropertyChanged();

            StatusText = AimMagicBulletsEnabled
                ? "Magic Bullets enabled"
                : "Magic Bullets disabled";

            SaveCheatSettings();
        }
    }

    public double AimFov
    {
        get => _aimFov;
        set
        {
            ...
            OnPropertyChanged();
            OnPropertyChanged(nameof(AimFovDisplay));

            SaveCheatSettings();

            StatusText = $"Aim FOV saved: {AimFovDisplay}";
        }
    }

    public string AimFovDisplay => $"{AimFov:0}°";

    public bool LootFullDropChance
    {
        get => _lootFullDropChance;
        set
        {
            if (_lootFullDropChance == value)
                return;

            _lootFullDropChance = value;
            OnPropertyChanged();

            StatusText = LootFullDropChance
                ? "100% Loot Drop Chance enabled"
                : "100% Loot Drop Chance disabled";
    ...
    public double MovementSpeedMultiplier
    {
        ...
        set
        {
            ...
            if (_movementSpeedMultiplier == rounded)
                return;
    ...
    private async Task SelectGamePathAsync()
    {
        ...
        {
            Items.Clear();
            StatusText = "Invalid folder. Select the folder that contains Remnant2-Win64-Shipping.exe or Remnant2-WinGDK-Shipping.exe, with Remnant2Unlocker installed under Mods\\ (or ue4ss\\mods\\ on some Game Pass setups)";
            return;
        }
    ...
    private const int MaxConcurrentImageLoads = 6;

    private async Task LoadImagesInBackgroundAsync()
    {
        using var throttle = new SemaphoreSlim(MaxConcurrentImageLoads);
    ...
    private void InitializeWeaponModBoostGroups(Dictionary<string, Dictionary<string, double>> saved)
    {
        WeaponModBoostGroups.Clear();
    ...
    private Dictionary<string, Dictionary<string, double>> BuildWeaponModBoostSettings()
    {
    ...
    {
        _weaponModBoostSettingsService.Save(BuildWeaponModBoostSettings());
    }
    ...
    private HotkeySettings BuildHotkeySettings() => new()
    {
    ...
    {
        _hotkeySettingsService.Save(BuildHotkeySettings());
    }

    // Bulk-apply from a loaded SettingsProfile. Sets backing fields directly and persists once at
    // the end, rather than going through each public property setter (which would each trigger
    // their own SaveCheatSettings()/SaveHotkeySettings() round trip) -- then a single
    // OnPropertyChanged(string.Empty) tells WPF to re-read every binding on this view model at
    // once, which is how a bulk change like this is meant to be announced.
    private void ApplyCheatSettings(CheatSettings settings)
    {
        _infiniteHealth = settings.InfiniteHealth;
        _infiniteStamina = settings.InfiniteStamina;
        _infiniteAmmo = settings.InfiniteAmmo;
        _noFallDamage = settings.NoFallDamage;
        _aimMagicBulletsEnabled = settings.AimMagicBulletsEnabled;
        _aimFov = settings.AimFov <= 0 ? 5.0 : settings.AimFov;
    ...
    private void LoadSettingsProfile()
    {
        ...
        try
        {
            ...
            if (profile.Hotkeys != null)
                ApplyHotkeySettings(profile.Hotkeys);
