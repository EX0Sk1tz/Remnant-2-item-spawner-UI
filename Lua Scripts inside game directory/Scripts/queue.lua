...
local MOD_DIR = require("paths").ModDir
...
-- Threading rule for this bridge: the command check and safe-spawn steps run in ExecuteWithDelay
-- callbacks (UE4SS's async thread) and hand only the actual summon/console command to the game
-- thread. Nothing here may call ExecuteInGameThread from inside an ExecuteInGameThread callback:
-- running the bridge itself on the game thread did that, and the game crashed in UE4SS's
-- ProcessEvent callback dispatch on the very first console command.
...
local ProcessNextSafeItem

local function ScheduleNextSafeItem()
    ExecuteWithDelay(safeDelayMs, function()
        -- The previous summon hasn't run on the game thread yet (loading screen, long frame):
        -- wait another round instead of queuing a second one behind it.
        if not cancelRequested and not Spawner.IsIdle() then
            ScheduleNextSafeItem()
            return
        end

        -- An error here would otherwise end the batch silently with the bridge stuck "busy".
        local ok, err = pcall(ProcessNextSafeItem)

        if not ok then
            FinishSafeQueue(
                "Safe spawn stopped at " .. tostring(status.processedCount) .. " / " .. tostring(status.totalCount),
                tostring(err))
        end
    end)
end

ProcessNextSafeItem = function()
...
local function StartSafeSpawn(paths, label, command)
    ...
    print("[Remnant2Unlocker] Delay per item: " .. tostring(safeDelayMs) .. "ms (no actor tracking)")
...
-- One path per line. Long lists come this way instead of inside command_queue.json, so the
-- command file Tick keeps re-reading stays tiny (see QueueWriter.SpawnManyAsync in the app).
local function ReadPathList(fileName)
    local file = io.open(MOD_DIR .. fileName, "r")
    if not file then return nil end
...
local function SpawnManySafe(command)
    ...
    local pathsFile = GetCommandValue(command, "pathsFile", "PathsFile")

    if pathsFile then
        pathsFile = tostring(pathsFile)

        -- Only a plain file name inside the mod folder.
        if pathsFile:find("[/\\]") or pathsFile:find("..", 1, true) then
            FinishSafeQueue("Spawn list rejected: " .. pathsFile, "Invalid spawn list name")
            return
        end
        ...
        if not paths then
            FinishSafeQueue("Spawn list not found: " .. pathsFile, "Missing spawn list")
            return
        end
    end
...
-- Content of the last command file that parsed. The file only changes when the app sends
-- something, so there's no need to re-parse it 5x/second on UE4SS's async thread in between.
local lastQueueContent = nil

function Queue.Tick()
    local content = ReadAllText(QUEUE_PATH)

    if not content then
        lastQueueContent = nil
        SetStatus("command_queue.json not found", "Missing command_queue.json")
        return
    end

    if content == lastQueueContent then
        return
    end
    ...
    lastQueueContent = content
    ProcessCommand(command)
end

-- Every 200ms. An error in one command can't stop the bridge from polling.
local function ScheduleTick()
    ExecuteWithDelay(200, function()
        local ok, err = pcall(Queue.Tick)

        if not ok then
            -- A running batch still owns the bridge; otherwise don't leave it stuck "busy".
            if #safeQueue == 0 then
                isBusy = false
            end

            SetStatus("Bridge error while processing command", tostring(err))
        end
