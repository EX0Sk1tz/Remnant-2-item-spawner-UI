...
### `OwnedItemsService` / `CollectionTracker` (collection tracking)
Drives the "what am I still missing" view. `inventory_cheats.lua` writes `owned_items.json` (`{ "scannedAt": <unix s>, "items": ["Weapon_AlphaOmega_C", ...] }`, the class name of every inventory item) on its 5s auto-scan and on the `scan_inventory` console command; a failed inventory read never overwrites the last good file. `OwnedItemsService.Refresh()` re-reads it only when its write time changes and treats anything unparseable (e.g. caught mid-write) as "no new data". `CollectionTracker` is the pure matching logic: items are matched on the lowercased class name (`ToClassKey`, the part after the last `.` of the path), and only types that stay in the inventory are tracked (`TrackedTypes`: weapons, armor, amulets, rings, mutators, relics). `MainViewModel` polls every 2s, sets `RemnantItem.IsOwned`, fills the sidebar counts (`CategoryTypeEntry.ProgressText`), and backs the "Missing only" filter, the collection bar and "Spawn Missing" (`QueueWriter.SpawnManyAsync` → the Lua `spawn_many_safe` action). `SpawnManyAsync` writes the paths to `spawn_list.txt` (one per line) and the command only names that file: a ~40 KB `command_queue.json` (all ~400 missing items) crashed the game inside UE4SS's ProcessEvent hook, and `Queue.Tick` now also skips re-parsing an unchanged command file. Threading rule (learned from four crash dumps, all inside UE4SS's `ExecuteInGameThread` dispatch in its ProcessEvent hook): the bridge runs in `ExecuteWithDelay` callbacks and only hands the summon/console command itself to the game thread; nothing may call `ExecuteInGameThread` from inside an `ExecuteInGameThread` callback (running the bridge on the game thread did that and crashed on the first console command). A safe-spawn step skips a round while the previous summon is still queued (`Spawner.IsIdle()`), so summons can't pile up behind a loading screen. Batch summons pass `track=false`, which skips the two `FindAllOf` actor scans per item (DestroyLastSpawned tracking); single spawns keep them. "Spawn Missing" uses the same 500ms gap as "Spawn Group": 100ms and ~270ms batches crashed after 1-10 items. Tests: `LuaSafeSpawnPacingTests` runs `queue.lua`/`spawner.lua` with a fake clock and game thread and asserts there's never a nested `ExecuteInGameThread` call. Covered by `Tests/Remnant2UnlockerApp.Tests`, including `LuaInventoryScanTests`, which runs the real `inventory_cheats.lua` under MoonSharp with a fake UE4SS.

### `DlcCatalog` (DLC badge and content filter)
Maps an item to its DLC from its asset path: `/Game/World_DLC1/` = The Awakened King, `World_DLC2` = The Forgotten Kingdom, `World_DLC3` = The Dark Horizon; everything else is base game (an unknown `World_DLCn` folder still counts as DLC). Exposed on `RemnantItem.Dlc` for the card badge. `MainViewModel.ContentFilter` (All content / Base game / one DLC) scopes the item list, the collection counts and "Spawn Missing" to the same set of items; searching a DLC name also finds its items. There is deliberately no world filter: 44% of items live in the shared `World_Base` folder regardless of the world they're found in, so a path-based world filter would silently miss them.
...
- Methods invoked directly from view code-behind (not commands): `LoadAsync`, `SpawnItemAsync`, `ForceConsoleSpawnItemAsync`, `CopySummonCommand`, `SummonTraitAsync`, `AddTraitAsync`, `BuildWikiUrl`, `RefreshDiagnosticsAsync`.
- As of 4.4.0, `RefreshDiagnosticsAsync`/`RefreshSummonableTraitsStateAsync` run their disk I/O (`DiagnosticsService.Run`, `SummonableTraitsService.IsInstalled`) via `Task.Run` and are awaited everywhere, instead of blocking the UI thread synchronously on every spawn action.
...
- **Action dispatch** (`ProcessCommand`'s `action` field, all case-sensitive, accepts both `lowerCamel` and `PascalCase` JSON keys via `GetCommandValue`):
  ...
  | `spawn_many` / `spawn_many_safe` | `SpawnManySafe` → reads a `paths`/`Paths` array and does the same delayed-loop spawn as group unlock | ✅ `spawn_many_safe` only, via `QueueWriter.SpawnManyAsync` ("Spawn Missing"); `spawn_many` is still never sent |
...
### `Scripts/spawner.lua`
`Spawner.SpawnPath(path, dropQuantity, stackSize, itemLevel)` — normalizes the path (trims, strips `|`, strips a leading `"summon "` if present, and drops any trailing arguments baked into the `items.json` path such as Prism Fragments' ` 1 1 31`), builds `"summon <path> <drop> <stack> [<level>]"`, and runs it via `ExecuteInGameThread` + `ksl:ExecuteConsoleCommand`. Some Cheat Mod builds (seen on Game Pass) only accept a bare `summon <path>`. If the first argument-carrying summon of a session produces no new actor of that class, it is retried as a plain `summon <path>` repeated `<drop>` times. If that works, the session switches to plain mode (stack size and level can't be applied). If neither is detected, the fallback is disabled for the session so nothing double-spawns. `Spawner.SpawnItem` is a convenience wrapper with its own relic-fragment level-31 check, but nothing in the mod calls it — `queue.lua` always calls `SpawnPath` directly with a level already resolved.
...
Polls `hotkeys.json` for `movementSpeedMultiplier` every **1000ms** (`LoopAsync(1000, ReloadSettings)`) — unlike `hotkeys.lua`, this one *does* pick up Settings changes without a game restart. Every **250ms** (`LoopAsync(250, ApplyMovementSpeed)`) it finds the player pawn (`UEHelpers.GetPlayerController().Pawn`, falling back to `FindFirstOf("RemnantPlayerCharacter")`), locates a movement component (tries `CharacterMovement`, `MovementComponent`, `LocomotionComponent` in order), and sets `MaxWalkSpeed = baseSpeed * multiplier`. Remnant 2 writes `MaxWalkSpeed` itself on every movement-state change (0 standing still, 175 walking, 300 jogging, confirmed from in-game traces) and leaves it alone in between, so any value that differs from the one the script last wrote is taken as the new `baseSpeed` for the current state; this keeps the game's walk/jog ratios instead of forcing one fixed speed. A reading of 0 (idle) is skipped silently. When the multiplier drops back to `1.0`, `ResetMovementSpeedIfNeeded` restores `baseSpeed` once, but only if `MaxWalkSpeed` still holds the script's own value.
