...
public sealed class DiagnosticsService
{
    ...
    /// <summary>
    /// Runs all setup checks. <paramref name="summonableTraitsInstalled"/> lets a caller that already
    /// computed this (e.g. MainViewModel.RefreshSummonableTraitsState) pass it in, avoiding a second
    /// full scan of the Mods folder for the same answer.
    /// </summary>
    public DiagnosticReport Run(bool? summonableTraitsInstalled = null)
    {
        ...
        var modsPath = GamePathService.ResolveModsFolder(path);

        CheckUE4SSCore(report, path, modsPath);
        ...
        CheckDirectory(
            ...
            "Mods folder",
            "Mods folder missing",
            "The selected Win64 folder does not contain a Mods folder (checked both Mods\\ and ue4ss\\mods\\ for Game Pass installs).",
        ...
        CheckSummonableTraits(report, summonableTraitsInstalled);
    ...
    private static void CheckUE4SSCore(DiagnosticReport report, string path, string modsPath)
    {
        var isNestedLayout = string.Equals(
            modsPath.TrimEnd('\\', '/'),
            Path.Combine(path, "ue4ss", "mods").TrimEnd('\\', '/'),
            StringComparison.OrdinalIgnoreCase);

        var ue4ssFolder = isNestedLayout ? Path.Combine(path, "ue4ss") : path;
        var ue4ssDll = Path.Combine(ue4ssFolder, "UE4SS.dll");
        var settingsIni = Path.Combine(ue4ssFolder, "UE4SS-settings.ini");

        if (File.Exists(ue4ssDll))
        {
            AddPass(report, "UE4SS core installed", "Found UE4SS.dll.", ue4ssDll);
        }
        else
        {
            AddIssue(
                ...
                "UE4SS.dll missing",
                "UE4SS not installed",
                "UE4SS itself (the modding framework everything else depends on) was not found. This is often caused by antivirus/Windows Defender quarantining it, since it's a DLL that hooks the game.",
                "Reinstall UE4SS into this folder, and check your antivirus quarantine/history if it disappears again.",
                ue4ssDll,
                DiagnosticStatus.Error);
        }
        ...
        CheckFile(
            ...
            settingsIni,
            "UE4SS-settings.ini",
            "UE4SS settings file missing",
            "UE4SS-settings.ini was not found next to UE4SS.dll.",
            "Reinstall UE4SS so it ships its default settings file.",
            false);

        if (!isNestedLayout)
            return;

        var dwmapiDll = Path.Combine(path, "dwmapi.dll");

        if (File.Exists(dwmapiDll))
        {
            AddPass(report, "Game Pass proxy DLL (dwmapi.dll)", "Found dwmapi.dll next to the game executable.", dwmapiDll);
            return;
        }
        ...
        AddIssue(
            report,
            "dwmapi.dll missing",
            "Game Pass proxy DLL missing",
            "This looks like a Game Pass install using the nested ue4ss\\mods layout, but dwmapi.dll (the proxy DLL that loads UE4SS on game start) is missing from the WinGDK folder. Without it, UE4SS never loads at all.",
            "Reinstall UE4SS for Game Pass (the proxy DLL build) so dwmapi.dll ends up directly next to Remnant2-WinGDK-Shipping.exe.",
            dwmapiDll,
            DiagnosticStatus.Error);
    }
    ...
    {
        var modsPath = GamePathService.ResolveModsFolder(win64Path);
        var modsTxt = Path.Combine(modsPath, "mods.txt");
        var enabledTxt = Path.Combine(modsPath, "enabled.txt");
    ...
    {
        if (summonableTraitsInstalled ?? _summonableTraitsService.IsInstalled())
        {
            AddPass(
                report,
    ...
    private static string? FindLatestLog(string win64Path)
    {
        ...
        // Some Game Pass UE4SS builds run out of a nested "ue4ss" folder and log there instead.
        var nested = Path.Combine(win64Path, "ue4ss", "UE4SS.log");
        ...
        if (Directory.Exists(ue4ssDir))
            candidates.AddRange(Directory.GetFiles(ue4ssDir, "*.log", SearchOption.TopDirectoryOnly));
